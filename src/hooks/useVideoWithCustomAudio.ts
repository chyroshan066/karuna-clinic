...
export const useVideoWithCustomAudio = ({
  ...
  // Effect to permanently disable video audio
  useEffect(() => {
    if (!enabled) return;
    ...
    // Permanently disable video audio with multiple safeguards
    video.muted = true;
    video.volume = 0;
    
    // Set these attributes directly on the element to prevent any audio
    video.setAttribute('muted', 'true');

    // Prevent ANY changes to video muted/volume state
    ...
    // Cleanup event listeners and restore properties on unmount
    return () => {
      video.removeEventListener('play', handlePlay);
      video.removeEventListener('pause', handlePause);
      video.removeEventListener('seeked', handleSeeked);
